...
public class TemporadaController {
	...
	public Boolean validar() {
            ...
            if(!temporadas.isEmpty()) {
                 ...
                 return "finaliada".equals(te.getEstado()); 	
            }
           return true;
	}
  	...
  	return tem.getEstado().equalsIgnoreCase("llenando");
  	}
	...
	public List<TemporadaA> Antiguo() {
		...
		if(!temporadas.isEmpty()) {
                    Temporada te=temporadas.stream().max((x,y)->x.getNumero().compareTo(y.getNumero())).get();
                    boolean b=!te.getEstado().equalsIgnoreCase("finalizada");
                    if(b) {
	...
	public Temporada registrar(@RequestBody Temporada tem) {
            ...
            if(!temporadas.isEmpty()){
           Temporada te=temporadas.stream().max((x,y)->x.getNumero().compareTo(y.getNumero())).get();
           b=te.getEstado().equalsIgnoreCase("finalizada");
            tem.setNumero(te.getNumero()+1);
          }
            if(b) {
            	 tem.setEstado("llenando");
